...
going to need computers to really explore such a complicated space!
...
We can do better than looking at *only* position `38` in the signvector. Why don't we compare the entire signvectors at once? There
...
In a surprise upset, the quantization with the closest signvector to the `overtone_scale` is apparently sc7-27 as represented by
its mode (0, 2, 4, 5, 7, 8, 9). This doesn't mean that sc7-27 is a *better* quantization than the `acoustic_scale`, just that it's
one that better reflects the structural irregularity of the `overtone_scale`. The modes of melodic minor simply have too much regularity--
...
one vary its individual scale degrees while maintaining the same overall structure? A minimally constrained heptachord has six degrees of 
...
20), and sc7-31 is only a little more flexible. This is a consequence of quantizing to twelve-tone equal temperament, which simply doesn't have
...
```{r}
all_12_heptachords <- sapply(1:38, sc, card=7)
heptachord_freedoms <- apply(all_12_heptachords, 2, howfree)
table(heptachord_freedoms)
```

This table shows that there are 11 set classes with 1 degree of freedom and 27 set classes with 2 degrees of freedom. None are more free than that.
...
triad (0, 3.5, 7). The neutral triad has only three sizes of step (3.5 and 5) as opposed to the three step sizes for major and minor (3, 4, and 5).
...
by (0, 2, 4, 5, 7, 9, 11) in 12edo. The just and meantone scales are similar in many ways, but the meantone scale elides differences that the just
...
scale to match, since most conceivable sign vectors don't correspond to scales that can actually exist. We could explore various options, but
ultimately we don't have to: part of MCT is a map of all the colors that exist (for scale sizes up to heptachords) and their adjacency relationships. 

This map involves datasets that are too large to distribute with the `musicMCT` package. Two of them can be downloaded from the [GitHub repository
...
Additionally, the adjacency relationships between all these colors have been computed in the file `color_adjacencies.rds` (~130 MB) which is too
large to host directly on GitHub. It can be downloaded from GoogleDrive [at this link](https://drive.google.com/file/d/1ektff4Hoyg43tB6PewfLNDQyPbWPi6Fi/view?usp=drive_link).
...
color in a MCT hyperplane arrangement ("Modal Color Theory," 28). If you try to lookup a scale's color number without the `representative_signvectors`
...
This tells us that the `overtone_scale` starts with its 6th largest step and then ascends through increasingly smaller step sizes, until
its final step from $\hat{7}$ to $\hat{1}$ uses the scale's largest step size.

Our `best_simple_approximation` reduces most of the variety out of the step sizes: all the steps from its $\hat{1}$ up to $\hat{6}$ are equal.
...
any of its structure. By contrast, our original `acoustic_scale` does outright contradict aspects of the `overtone_scale`'s structure. For instance,
...
to $\hat{6}$ is smaller (because all its step sizes get progressively smaller until the last one). We can see that contradiction by comparing the 
...
The `acoustic_scale` is definitely the odd one out here. It's also, in terms of voice-leading distance, not clearly closer to the `overtone_scale`
than our `best_simple_approximation` is:


```{r}
minimizeVL(overtone_scale, acoustic_scale)
minimizeVL(overtone_scale, best_simple_approximation)
```
...
the `overtone_scale`. But of course things are never that simple! The best approximation for a scale depends on your goals and what you're trying
to approximate for. The `acoustic_scale` is indeed a decent approximation in some senses, especially if you're limited to the familiar 12 pitch-classes
of the piano. But hopefully this vignette shows how we might, operating under different priorities, come to other conclusions--and how Modal Color Theory
and `musicMCT` can guide us in that search.

# Coda: Brightness graphs

We've been doing everything numerically so far, but another tool from "Modal Color Theory"

The most important function is [brightnessgraph()], which lets us visualize the voice-leading relationships between
a scales modes (and thus to some degree the internal structure of the scale, as described in "Modal Color Theory," 7-11).
...
of our figure here is mode IV whereas it's listed as mode VII in the *JMT* article. That's just because the article treats melodic minor
...
```{r, fig.width=5, fig.height=5, fig.fullwidth=TRUE}
brightnessgraph(overtone_scale, numdigits=1)
```

This looks rather haphazard! The graph is less connected (i.e. there are fewer arrows) and the vertical placement of its
...
```{r, fig.width=5, fig.height=5, fig.fullwidth=TRUE}
brightnessgraph(quantized_overtone_color$set, edo=30)
```
...
Finally, let's take a look at the brightness graph for our `best_simple_approximation`:

```{r, fig.width=5, fig.height=5, fig.fullwidth=TRUE}
brightnessgraph(best_simple_approximation, show_pitches=FALSE)
```
